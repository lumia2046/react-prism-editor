...
class Editor extends React.Component {
  ...
  getLanguage(language) {
    if (basicLanguages.includes(language)) {
      ...
      console.warn(
        "Your input language is not support, use default language javascript"
      );
      return "javascript";
    }
  }
  ...
  handleClick(evt) {
    ...
    this.selection = getCaretPosition(this.pre);
  }
  getPlain() {
  ...
  restoreStackState(offset) {
    ...
    const { plain, selection } =
      this.undoStack[this.undoStack.length - 1 - offset];
  ...
  handleKeyDown(evt) {
    if (evt.keyCode === 9 && !this.ignoreTabKey) {
      ...
      // Backspace Key
      const { start: cursorPos, end: cursorEndPos } = getCaretPosition(
        this.pre
      );
      ...
      // Enter Key
      const { start: cursorPos } = getCaretPosition(this.pre);
  ...
  onPaste = (e) => {
    e.preventDefault();
    const currentCursorPos = getCaretPosition(this.pre);
  ...
  setPrismContent(code, language) {
    ...
    this.setState(
      ...
      () => {
        ...
        this.Prism.hooks.run("complete", {
          language,
          code,
          element: this.pre.querySelector("code"),
        });
        // this.styleLineNumbers()
      }
    );
  ...
  componentDidUpdate() {
    if (this.selection) {
      setCaretPosition(this.pre, this.selection);
    }
  }
  ...
  render() {
    const { language, readOnly, theme, lineNumber, clipboard, showLanguage } =
      this.props;
    const { content, lineNumbersHeight, id } = this.state;
    return (
      <div className={`prism-editor-${id} module-theme-${theme}`}>
        {/* {lineNumber && <div
